...
public class MyTable implements Table {
    ...
    public MyTable(String name, List<Class<?>> colTypes, MyTableProvider providerRef) {
        this.tableName = name;
    ...
    public int commit() {
        ...
        modifiedData.clear();
        size = oldData.size();
        MyTableBuilder tableBuilder = new MyTableBuilder(provider, this);
        save(tableBuilder);
        uncommittedChanges = 0;
    ...
    public boolean save(MyTableBuilder tableBuilder) {
        if (oldData == null) {
            return true;
        }
        if (tableName.equals("")) {
        ...
        for (int i = 0; i < 16; ++i) {
            ...
            if (isDirEmpty) {
                try {
                    if (path.exists()) {
                        MyTableProvider.recRemove(path);
                    }
                } catch (IOException e) {
                    return false;
                }
            ...
            if (path.exists()) {
                File file = path;
                try {
                    if (!MyTableProvider.recRemove(file)) {
    ...
    public boolean saveTable(Set<String> keys, String path, MyTableBuilder tableBuilder) throws IOException {
        if (keys.isEmpty()) {
            try {
