...
public abstract class CommandSource {
	...
	private Command createCommand(ShellReceiver receiver) throws ShellException {
		...
		try {
			...
			do {
				try {
					availableConstructors.add(tmpCommandClass.getConstructor(ShellReceiver.class));
			...
			for (Constructor<?> constructor : availableConstructors) {
				if (constructor.getParameterTypes().length == 1 &&
						constructor.getParameterTypes()[0].equals(ShellReceiver.class))
				{
			...
			throw new ShellException(invalidCommandName + ": invalid command.");
		}
		if (retValue != null) {
			readArgs(retValue);
		}
