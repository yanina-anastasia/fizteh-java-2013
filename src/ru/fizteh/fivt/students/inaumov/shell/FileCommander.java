...
public class FileCommander {
	...
	public void createNewDirectory(String newDirectoryName) {
		...
		if (newDirectoryFile.exists()) {
			throw new IllegalArgumentException("cannot create directory " + newDirectoryName + ": file exists");
		}
	...
	public void moveFiles(String sourceFileName, String destinationFileName) throws IOException {
		...
		if (sourcePath.toFile().equals(destinationPath.toFile())) {
			throw new IllegalStateException("cannot move " + sourceFileName + " to " + destinationFileName + ": files are same");
		}
		if (sourcePath.toFile().isDirectory() && destinationPath.toFile().isFile()) {
			throw new IllegalStateException("cannot overwrite non-directory" + destinationFileName + " with directory " + sourceFileName);
		}
	...
	public void copyFiles(String sourceFileName, String destinationFileName) throws IOException {
		...
		if (sourcePath.toFile().equals(destinationPath.toFile())) {
			throw new IllegalStateException("cannot copy " + sourceFileName + " to " + destinationFileName + ": files are same");
		}
