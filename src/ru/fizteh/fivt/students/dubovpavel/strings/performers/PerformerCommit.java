...
public class PerformerCommit<D extends Dispatcher & StorageAccessible<Storage<MindfulDataBaseMultiFileHashMap>>> extends Performer<D> {
    ...
    public void execute(D dispatcher, Command command) {
        try {
            dispatcher.callbackWriter(Dispatcher.MessageType.SUCCESS, String.valueOf(dispatcher.getStorage().getCurrent().commit()));
